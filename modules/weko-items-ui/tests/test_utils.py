...
def test_make_stats_file_issue43174(app, db, users, db_itemtype_checkbox, db_records_checkbox):
    ...
    with app.test_request_context():
        with patch("flask_login.utils._get_user", return_value=users[2]["obj"]):
            ...
            recids_reverse = copy.deepcopy(recids)
            recids_reverse.reverse()

            res = make_stats_file(item_type_id, recids_reverse, permissions)
            assert res[0][1][-4] == "チェックボックステスト.値[0]"
            assert res[1]["1"][-4] == "A"
            assert res[1]["2"][-4] == "A"
            assert res[0][1][-3] == "チェックボックステスト.値[1]"
            ...
            res = make_stats_file_with_permission(
                item_type_id, recids_reverse, records_metadata, permissions)
            assert res[0][1][-4] == "チェックボックステスト.値[0]"
            assert res[1]["1"][-4] == "A"
            assert res[1]["2"][-4] == "A"
            assert res[0][1][-3] == "チェックボックステスト.値[1]"
            ...
            res = make_stats_file(item_type_id, ["3"], permissions)
            assert  "チェックボックステスト.値[0]" in res[0][1]
            assert  "チェックボックステスト.値[1]" not in res[0][1]

            record_without_checkbox = WekoRecord.get_record_by_pid(3)
            res = make_stats_file_with_permission(
                item_type_id,
                ["3"],
                {"3": record_without_checkbox},
                permissions
            )
            assert  "チェックボックステスト.値[0]" in res[0][1]
            assert  "チェックボックステスト.値[1]" not in res[0][1]
            ...
            with patch("weko_records_ui.views.escape_newline",
